...
public class ArenaGame {
	...
	private void deleteWorld() {
		// removes all players from the world first. normally, no players should be in
		// the world anymore
	...
	// adds an ArenaPlayer object to the players HashMap. Returns true if a new
	// element was added, otherwise false.
	...
	// removes an ArenaPlayer object from the players HashMap. Returns true if an
	// element was removed, otherwise false.
	public boolean removePlayer(Player bukkitPlayer) {
		int index = hasPlayer(bukkitPlayer);
		if (index == -1) // No such player in the game!
			return false;
		players.get(index).teleportBack(); // teleports the player back to the old world and loads his values from the
											// backup.
